...
export function CalendarClient({ items }: CalendarClientProps) {
  ...
  const monthDays = useMemo(() => anchorDate ? monthGridDays(anchorDate) : [], [anchorDate]);
  const currentMonth = anchorDate?.getMonth() ?? 0;
  ...
  return (
    <div className="space-y-6" dir={isRtl ? 'rtl' : 'ltr'}>
      {/* Header */}
      <div className="rounded-3xl border border-black/6 bg-surface-elevated/92 p-5 shadow-[0_24px_64px_rgba(93,107,107,0.07)] ring-1 ring-white/70 sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-baseline gap-3">
                <h1 className="text-2xl font-black text-[#5D6B6B] sm:text-3xl">
                  {anchorDate ? formatMonthTitle(anchorDate, isRtl) : 'Loading...'}
                </h1>
                {clock.clock ? (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-black/8 bg-[#F1F7F7] px-3 py-1 text-xs font-bold text-foreground/55">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-xl border border-black/8 bg-white text-foreground/55 shadow-sm transition hover:border-[#F7CBCA]/25 hover:text-[#F7CBCA]"
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[180px] max-w-xs">
            ...
            {searchQuery ? (
              <button
                ...
                className="absolute end-2 top-1/2 -translate-y-1/2 text-foreground/35 hover:text-[#F7CBCA]"
              >
              ...
              <button
                ...
                className={cn(
                  ...
                  typeFilter === f.value
                    ...
                    : 'border-black/8 bg-white text-foreground/55 hover:border-[#F7CBCA]/20 hover:text-[#F7CBCA]'
                )}
              >
                {f.label}
              </button>
          ...
          {(typeFilter !== 'all' || statusFilter !== 'all' || searchQuery) ? (
            <button
              ...
              className="inline-flex items-center gap-1 rounded-xl border border-black/8 bg-white px-3 py-1.5 text-xs font-bold text-foreground/45 transition hover:text-[#F7CBCA]"
            >
              <X className="h-3.5 w-3.5" />
              Clear
            </button>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-black/7 bg-[#F1F7F7]/80 px-3 py-1 text-xs font-bold text-foreground/55">
      ...
      <div className="hidden sm:grid sm:grid-cols-7 sm:gap-2">
        {(isRtl ? [...WEEKDAYS_SHORT_AR].reverse() : WEEKDAYS_SHORT).map((day) => (
          <div
            key={day}
            className="rounded-2xl px-3 py-2 text-center text-xs font-black uppercase tracking-[0.12em] text-foreground/40"
          >
            {day}
          </div>
        ))}
      ...
      {filteredItems.length === 0 && typeFilter === 'all' && statusFilter === 'all' && !searchQuery ? (
        <div className="rounded-3xl border border-black/6 bg-surface-elevated/88 p-12 text-center shadow-[0_20px_54px_rgba(93,107,107,0.06)]">
          ...
          <p className="mt-2 text-sm leading-6 text-foreground/55">Schedule content from Content Studio to see it here.</p>
          <Link
            href="/dashboard/content-studio"
                ...
                className={cn(
                  ...
                  isSelected
                    ...
                    : isToday
                      ...
                      : dayItems.length > 0
                        ? 'border-black/8 bg-surface-elevated/85 shadow-sm'
                        : 'border-black/5 bg-surface-elevated/60'
                )}
              >
                ...
                <div className="mt-1.5 hidden space-y-1 sm:block">
                  {dayItems.slice(0, 2).map((item) => {
                    const dotColor = statusDotColors[item.status] || 'bg-foreground-muted';
                    return (
                      <div
                        key={item.id}
                        className="flex min-w-0 items-center gap-1.5 rounded-lg px-1.5 py-1 text-[11px] font-bold leading-tight text-foreground/65 transition group-hover:bg-black/[0.03]"
                      >
      ...
      {selectedDay ? (
        <>
          ...
          <div
            ref={popoverRef}
            className={cn(
              'fixed bottom-0 start-0 end-0 z-40 rounded-t-3xl border border-black/6 bg-surface-elevated/95 p-5 shadow-[0_-12px_48px_rgba(93,107,107,0.12)] backdrop-blur-xl transition-all sm:static sm:rounded-3xl sm:border sm:bg-surface-elevated/90 sm:p-6 sm:shadow-[0_20px_54px_rgba(93,107,107,0.07)]'
            )}
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                ...
                className="flex h-8 w-8 items-center justify-center rounded-xl border border-black/8 bg-white text-foreground/45 transition hover:text-[#F7CBCA]"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            {selectedItems.length === 0 ? (
              <div className="mt-5 rounded-2xl border border-dashed border-black/8 bg-[#F1F7F7]/60 p-6 text-center">
                <p className="text-sm font-bold text-foreground/50">No scheduled items on this day</p>
                <Link
                  href="/dashboard/content-studio"
                    ...
                    >
                      <div className="flex min-w-0 items-start gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-medium text-foreground/50">
