...
const taskDataLog = logger.child('data:tasks');
...
export async function listTasks(
  ...
  taskDataLog.info('before list tasks', {
  ...
  if (!isSupabaseConfigured) {
    taskDataLog.warn('Supabase is not configured');
  ...
  taskDataLog.info('after list tasks', {
    workspaceId: options.workspaceId ?? null,
    count: data?.length ?? 0,
...
export async function bulkDeleteTasks(
  ...
  return emptyDataResult({ deleted: data?.length ?? taskIds.length }, true);
}
...
export async function bulkDuplicateTasks(
  ...
  const { data: originals, error } = await client
    .from('tasks')
    .select('id, agent_type, title, description, input_data, priority')
    .in('id', taskIds)
    .eq('workspace_id', workspaceId);
  ...
  const inserts = originals.map((task) => ({
...
export async function bulkAssignTasks(
  ...
  if (!isSupabaseConfigured) {
    return emptyDataResult({ updated: 0 }, false);
  }

  if (taskIds.length === 0) {
    return emptyDataResult({ updated: 0 }, true);
  }
  ...
  if (error) {
    return errorDataResult({ updated: 0 }, error.message);
  }

  return emptyDataResult({ updated: data?.length ?? taskIds.length }, true);
}
