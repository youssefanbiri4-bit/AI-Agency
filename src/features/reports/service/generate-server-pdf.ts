...
import { renderReportToHTML } from '@/features/reports/service/report-generator';
import type { ClientReport } from '@/features/reports/service/report-types';
import type { GenerateServerPdfOptions } from '@/features/reports/service/report-types';
import { CONCURRENCY_SLOTS, withConcurrencyLimit } from '@/lib/concurrency-limiter';

const pdfLog = logger.child('reports:server-pdf');

// Preserved legacy limit — now enforced through the shared concurrency limiter
// (PDF_GENERATION slot) so it can be observed/overridden alongside other slots.
...
export async function generateServerPDF(
  ...
  return startSpan(
    {
      op: 'pdf.generation',
      name: `PDF: ${report.title}`,
      attributes: {
        'pdf.client': report.cover.client,
        'pdf.sections': report.sections.length,
        'pdf.has_password': Boolean(options.password),
      },
    },
    async (span) => {
      const result = await withConcurrencyLimit(
        CONCURRENCY_SLOTS.PDF_GENERATION,
        async () => {
          const puppeteerBuffer = await renderPdfWithPuppeteer(report);
          let buffer = puppeteerBuffer ?? (await renderPdfWithPdfLib(report));

          if (puppeteerBuffer) {
            span.setAttribute('pdf.renderer', 'puppeteer');
          } else {
            span.setAttribute('pdf.renderer', 'pdf-lib');
          }
        ...
        // Fail-fast: keep the original behaviour of rejecting when all PDF slots
        // are busy rather than queueing a slow, resource-heavy browser launch.
        { maxConcurrency: MAX_CONCURRENT_PDF, failOnQueue: true }
      );
      span.setAttribute('pdf.bytes', result.length);
      return result;
    }
  );
}
